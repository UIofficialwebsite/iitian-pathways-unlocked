...
export function getGradeFormula(subjectKey: string): string {
  ...
  switch (subjectKey) {
    case "maths1": case "english1": case "computational": case "english2":
      return "max(0.6F + 0.3max(Qz), 0.45F + 0.25Qz1 + 0.3Qz2)";
    case "statistics1": case "statistics2":
      return "max(0.6F + 0.3max(Qz), 0.45F + 0.25Qz1 + 0.3Qz2) + Bonus";
    case "maths2":
      return "min(100, max(0.6F + 0.3max(Qz), 0.45F + 0.25Qz1 + 0.3Qz2) + Bonus)";
    case "python":
    ...
    case "business_analytics":
      return "0.2*(0.7max(Qz)+0.3min(Qz)) + A + F + Bonus";
    case "programming_python": case "databasems":
      return "GAA_Comp + 0.2*OP + 0.45*F + max(0.2max(Qz), 0.1Qz1+0.2Qz2)";
    case "appdev1": case "appdev2":
      return "0.05*(GLA/GAA) + max(0.6F+0.25max(Qz), 0.4F+0.25Qz1+0.3Qz2)";
    ...
    case "deep_learning_practice":
      return "0.05*GA + 0.15*Sum(Qz) + 0.25*NPPE_Avg + 0.25*Viva";
    ...
    case "advanced_algorithms":
      return "0.15*GAA + max(0.2Qz1+0.2Qz2+0.45F, 0.5F+0.25max(Qz))";
...
export function calculateDiplomaGrade(subjectKey: string, values: Record<string, number>): number {
  const { GAA = 0, GA = 0, Qz1 = 0, Qz2 = 0, F = 0, OPPE1 = 0, OPPE2 = 0, KA = 0, 
          ...
          GrPA = 0, GAA1 = 0, GAA2 = 0, LE = 0, LV = 0, WE = 0, ID = 0 } = values;
  ...
  switch (subjectKey) {
    ...
    case "ml_techniques":
      const mltEnd = Math.max(0.6 * F + 0.25 * Math.max(Qz1, Qz2), 0.4 * F + 0.25 * Qz1 + 0.3 * Qz2);
      return 0.05 * GAA + mltEnd + Bonus;
    ...
    case "business_analytics":
      const qzComp = (0.7 * Math.max(Qz1, Qz2) + 0.3 * Math.min(Qz1, Qz2)) * 0.2;
