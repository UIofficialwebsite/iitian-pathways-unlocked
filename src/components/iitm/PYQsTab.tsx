  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
        {contentLoading ? (
          <div className="col-span-3 flex justify-center py-20 animate-pulse text-slate-400">Loading assessments...</div>
        ) : filteredPYQs.length > 0 ? (
          filteredPYQs.map((pyq) => (
            <Card key={pyq.id} className="border border-slate-100 shadow-sm hover:shadow-md transition-all flex flex-col h-full bg-white group">
              <CardHeader className="flex-grow pb-4 px-5 pt-5">
                <div className="flex justify-between items-start mb-4">
                  {/* Exam Type Badge */}
                  <span className="px-2 py-0.5 bg-blue-50 text-indigo-600 text-[10px] font-bold uppercase rounded">
                    {pyq.exam_type}
                  </span>
                  {/* Year */}
                  <span className="text-[11px] font-medium text-slate-400">{pyq.year}</span>
                </div>
                <CardTitle className="text-[15px] font-bold text-slate-900 leading-tight mb-2 group-hover:text-indigo-600 transition-colors font-sans">
                  {pyq.title}
                </CardTitle>
              ...
              <CardFooter className="flex justify-between items-center border-t border-slate-50 px-5 py-4 mt-auto">
                <div className="flex items-center gap-2">
                  {/* Download Button - Black Pill */}
                  <button 
                    onClick={() => handleDownload(pyq.id, 'pyqs', pyq.file_link || undefined)}
                    disabled={!pyq.file_link}
                    className="flex items-center gap-2 bg-black text-white h-9 px-4 rounded-full hover:bg-slate-800 transition-colors shadow-sm disabled:opacity-50"
                  >
          ...
          <div className="col-span-full py-20 text-center text-slate-400 font-medium italic">
            No papers found for the selected subject/year.
          </div>
        )}
      </div>
    </div>
