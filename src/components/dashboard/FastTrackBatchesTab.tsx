      ...
      <div className="p-5 flex flex-col flex-1">
        ...
        <div className="min-h-[50px] space-y-2 mb-4">
          ...
          <div className="flex items-center gap-2.5 text-[#4b5563] text-[14px]">
            ...
            <span className="font-bold">Fastrack</span> 
...
const FastTrackBatchesTab: React.FC<FastTrackBatchesTabProps> = ({ focusArea, onSelectCourse }) => {
  ...
  useEffect(() => {
    const fetchCourses = async () => {
      ...
      const { data } = await supabase.from('courses')
        ...
        .eq('batch_type', 'fastrack'); // Filter changed to fastrack
  ...
  // Interleaved Keywords for "Unorganised" animated placeholder
  const searchKeywords = useMemo(() => {
    ...
    return result.length > 0 ? result : ["Physics", "Chemistry", "Fastrack", "JEE", "NEET"];
  }, [batches, focusArea]);

  // Interval for placeholder rotation (2.5s)
  useEffect(() => {
        ...
        <div className="h-[73px] flex items-center justify-between gap-4">
          <div className="flex items-center gap-3">
            ...
            <h1 className={`${isMobile ? 'text-[18px]' : 'text-[22px]'} font-bold tracking-tight text-[#1a1a1a]`}>
              {isViewingAllFree ? "Free Fastrack Batches" : "Fastrack Batches"}
            </h1>
          </div>
          <div className="relative flex-1 max-w-[250px] md:max-w-sm">
            <Search className="absolute left-3 top-1/2 -translate-y-1/2 w-4 h-4 text-[#9ca3af]" />
            <Input 
              // Same animated placeholder logic without quotes
            ...
            <>
              ...
              {filtered.length === 0 && (
                <div className="text-center py-24 bg-gray-50 rounded-2xl border border-dashed border-gray-200 w-full">
                  <p className="text-gray-400 text-lg font-medium">No fastrack results found matching your search.</p>
                </div>
              )}
            </>
          )}
...
export default FastTrackBatchesTab;
