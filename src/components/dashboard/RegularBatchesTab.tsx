...
const CourseCard: React.FC<{ course: Tables<'courses'>, onSelect: (id: string) => void }> = ({ course, onSelect }) => {
  ...
  return (
    <div className="w-full max-w-[360px] bg-white rounded-[20px] overflow-hidden shadow-sm border border-[#e0e0e0] flex flex-col transition-all hover:shadow-md">
      {/* Restored Original Header Banner */}
      <div className="bg-[#fffbeb] relative pt-[10px] text-center pb-2">
        ...
        <img src={course.image_url || ""} className="w-full block aspect-video object-cover" alt="Course" />
        <div className="absolute bottom-[10px] left-1/2 -translate-x-1/2 bg-[#facc15] text-black text-[11px] font-extrabold px-3 py-1 rounded-[6px] uppercase whitespace-nowrap shadow-sm">
...
const RegularBatchesTab: React.FC<RegularBatchesTabProps> = ({ focusArea, onSelectCourse }) => {
  ...
  return (
    <div className="flex flex-col h-full">
      ...
      {/* SCROLLABLE GRID */}
      <div className="flex-1 overflow-y-auto p-6 md:p-8 bg-[#f9f9f9]">
