...
const CourseCard: React.FC<{ 
  ...
  return (
    /* UI/UX Update: Reduced corner rounding (rounded-xl) and ensured full width/height */
    <div className="w-full bg-white rounded-xl overflow-hidden shadow-sm border border-[#e0e0e0] flex flex-col transition-all duration-300 h-full">
      {/* Intelligence: Removed padding and used aspect-video + object-cover.
        This ensures the image fills the entire frame perfectly, eliminating white bars 
        while keeping a professional 16:9 ratio.
      */}
      ...
      <div className="p-5 flex flex-col flex-1">
        <div className="flex justify-between items-center mb-[10px]">
          <span className="text-[#f97316] font-bold text-[11px] uppercase tracking-wider">{course.level || 'Academic'}</span>
        ...
        <h2 className="text-[18px] font-bold text-[#1f2937] mb-[15px] line-clamp-2 leading-tight h-[44px]">{course.title}</h2>
        ...
        <div className="min-h-[50px] space-y-2 mb-4">
          ...
          <div className="flex items-center gap-2.5 text-[#4b5563] text-[14px]">
            ...
            <span className="font-bold text-gray-800">Ongoing</span> 
        ...
        <div className="flex justify-between items-center mt-auto pt-[15px] border-t border-gray-100">
          ...
          <div className="flex gap-2">
            {/* Reduced button rounding to match the card theme */}
            ...
            <button onClick={() => onSelect(course.id)} className="bg-white border border-[#e5e7eb] w-10 h-10 rounded-lg flex items-center justify-center hover:bg-gray-50 transition-colors">
              ...
              {!isViewingAllFree && paidBatches.length > 0 && (
                <div className="mb-14">
                  <h2 className="text-[22px] font-bold tracking-tight text-[#111] uppercase mb-8 hidden md:block">
                    Popular Regular Batches
                  </h2>
                  {/* Intelligent Grid: Automatically shifts columns while ensuring cards fill width */}
