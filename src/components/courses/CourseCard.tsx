...
const CourseCard: React.FC<CourseCardProps> = ({ course, index }) => {
  ...
  return (
    ...
    >
      {/* Blue Online Tag from Reference */}
      <div className="absolute top-0 left-0 z-20">
        <div className="bg-[#1E40AF] text-white text-[10px] font-bold px-3 py-1 rounded-br-lg rounded-tl-[6px] uppercase">
          Online
        </div>
      </div>

      {/* Banner Section - Prevents Zoom with Aspect Ratio */}
      <div 
      ...
      <div className="flex items-end justify-between mb-[20px] px-1">
        ...
        {discountPercent > 0 && (
          <div className="bg-[#e6f7ef] text-[#1b8b5a] px-[12px] py-[8px] text-[11px] font-normal flex items-center gap-[6px] shrink-0" style={{ borderRadius: '6px' }}>
            <Tag className="w-[13px] h-[13px]" /> Discount of {discountPercent}% applied
          </div>
        )}
      </div>
      ...
      <div className="flex gap-[10px]">
        <button 
          className="flex-1 border-[1.5px] border-[#1E3A8A] text-[#1E3A8A] bg-white h-[42px] flex items-center justify-center text-[13px] font-normal uppercase transition-all hover:bg-blue-50"
        ...
        <button 
          className="flex-1 bg-[#1E3A8A] text-white border-none h-[42px] flex items-center justify-center text-[13px] font-normal uppercase transition-all hover:opacity-90"
          style={{ borderRadius: '8px' }}
          onClick={() => navigate(`/course/${course.id}`)}
        >
          Buy Now
