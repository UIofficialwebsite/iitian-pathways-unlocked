...
import { Loader2, Check, Download, Share2, HelpCircle } from 'lucide-react';
...
const EnrollmentReceipt = () => {
    ...
    const fetchEnrollment = async () => {
      ...
      try {
        const { data, error } = await supabase
          ...
          .eq('status', 'success') // Assuming we only show this for successful enrollments
          .single();
        ...
        toast({
          title: "Error",
          description: "Could not load enrollment details.",
          variant: "destructive",
        });
      } finally {
        setLoading(false);
  ...
  if (!enrollment) {
    return (
      <div className="min-h-screen flex flex-col items-center justify-center bg-[#f5f8ff] text-slate-600">
  ...
  const isFree = enrollment.amount === 0 || enrollment.amount === null;
        ...
        <div className="flex flex-col gap-6">
          ...
          <div className="bg-white rounded-xl border border-slate-200 shadow-sm p-6">
            <div className="flex justify-between text-sm text-slate-500 mb-5">
              <span>Order Id: {enrollment.id.slice(0, 8)}...</span>
              <span>Order Date: {formatDate(enrollment.created_at)}</span>
            </div>
            ...
            <div className="flex flex-col sm:flex-row justify-between items-center bg-[#fcfdfe] border border-slate-100 rounded-lg p-4 gap-4">
              <div className="flex gap-4 items-center w-full">
                <img 
                  src={enrollment.courses.image_url || "https://via.placeholder.com/90x60"} 
                ...
                <div className="flex-grow">
                  ...
                  <p className="text-sm text-slate-500 mb-1">
                    Price: <span className="text-emerald-600 font-medium">{isFree ? 'Free' : `₹${enrollment.amount}`}</span>
                  </p>
              ...
              <Link to={`/courses/${enrollment.courses.id}`} className="w-full sm:w-auto">
            ...
            <div className="mt-6 bg-[#fafbff] p-4 rounded-lg">
              ...
              <p className="text-base text-slate-800 font-medium">
                {enrollment.courses.end_date ? formatDate(enrollment.courses.end_date) : 'Lifetime Access'}
              </p>
            </div>
          </div>
        </div>
        ...
        <div className="flex flex-col gap-6">
          ...
          <div className="bg-white rounded-xl border border-slate-200 shadow-sm p-6">
            ...
            <div className="flex justify-between text-sm text-slate-500 mb-3">
              ...
              <span>{isFree ? '₹0' : `₹${enrollment.amount}`}</span>
            </div>
            <div className="flex justify-between text-sm text-slate-500 mb-3">
              <span>Discount</span>
              <span>₹0</span>
