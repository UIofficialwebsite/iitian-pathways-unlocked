...
const IITMBSPrep = () => {
  ...
  const renderDropdownContent = (type: typeof openDropdown) => {
    ...
    return (
      <>
        {/* Exam Prep Gradient Bar */}
        <div className="h-1.5 w-full bg-gradient-to-r from-[#6366f1] to-[#a855f7] rounded-t-xl mb-3 -mt-3 -mx-3"></div>
        <div className="max-h-[200px] overflow-y-auto mb-3 space-y-1 custom-scrollbar">
          {type === 'branch' && (activeTab === 'courses' ? ["All Branches", ...branches] : branches).map(branch => (
          ...
          {type === 'courseLevel' && availableCourseLevels.map(lvl => (
            <label key={lvl} className="flex items-center gap-2 p-1.5 hover:bg-[#f9fafb] rounded cursor-pointer text-xs text-gray-700">
          ...
          {type === 'courseSubject' && availableCourseSubjects.map(sub => (
          ...
          {type === 'notesSubject' && availableNotesSubjects.map(sub => (
            <label key={sub} className="flex items-center gap-2 p-1.5 hover:bg-[#f9fafb] rounded cursor-pointer text-xs text-gray-700">
              <input type="checkbox" checked={tempNotesSubjects.includes(sub)} onChange={() => toggleTempItem(sub, tempNotesSubjects, setTempNotesSubjects)} className="accent-[#6366f1] rounded" /> {sub}
            </label>
          ))}
          {type === 'coursePricing' && ['free', 'paid'].map((opt) => (
          ...
          {type === 'examType' && examTypes.map(type => (
        ...
        <div className="flex gap-2 pt-2 border-t">
          <button onClick={() => setOpenDropdown(null)} className="flex-1 py-1 text-[11px] font-semibold text-slate-500 rounded hover:bg-slate-50">Cancel</button>
          <button 
            onClick={() => {
            ...
            className="flex-1 py-1 text-[11px] font-semibold bg-[#6366f1] text-white rounded hover:bg-[#5255e0]"
          >
            Apply
          </button>
        </div>
  ...
  return (
    <div className="min-h-screen bg-[#fcfcfc] font-sans">
      ...
      <main className="pt-16">
        ...
        <div ref={filterRef} className={`w-full transition-shadow duration-300 ${isSticky ? 'fixed top-16 bg-white border-b shadow-md z-[9999]' : 'relative z-[9999]'}`}>
          ...
          <div className="bg-white border-b border-[#f3f4f6] min-h-[56px] relative z-[100]">
            <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
              <div className="flex flex-nowrap items-center gap-3 py-3 font-sans overflow-x-auto no-scrollbar">
                ...
                {activeTab === 'courses' ? (
                  <>
                    <button onClick={() => toggleDropdown('branch')} className={`px-4 py-1.5 border rounded-[30px] text-[12px] md:text-[13px] flex items-center transition-all dropdown-container bg-white ${selectedBranch !== "Data Science" ? 'border-black text-black font-semibold' : 'border-[#e5e7eb] text-[#374151]'}`}>
                      ...
                      Branch
                      <span className={`ml-2 w-0 h-0 border-l-[4px] border-r-[4px] border-t-[4px] transition-transform ${openDropdown === 'branch' ? 'rotate-180' : ''} ${selectedBranch !== "Data Science" ? 'border-t-black' : 'border-t-[#374151]'} border-l-transparent border-r-transparent`}></span>
                    </button>
                    <button onClick={() => toggleDropdown('courseLevel')} className={`px-4 py-1.5 border rounded-[30px] text-[12px] md:text-[13px] flex items-center transition-all dropdown-container bg-white ${selectedCourseLevels.length > 0 ? 'border-black text-black font-semibold' : 'border-[#e5e7eb] text-[#374151]'}`}>
                      ...
                      Level
                      <span className={`ml-2 w-0 h-0 border-l-[4px] border-r-[4px] border-t-[4px] transition-transform ${openDropdown === 'courseLevel' ? 'rotate-180' : ''} ${selectedCourseLevels.length > 0 ? 'border-t-black' : 'border-t-[#374151]'} border-l-transparent border-r-transparent`}></span>
                    </button>
                    <button onClick={() => toggleDropdown('courseSubject')} className={`px-4 py-1.5 border rounded-[30px] text-[12px] md:text-[13px] flex items-center transition-all dropdown-container bg-white ${selectedCourseSubjects.length > 0 ? 'border-black text-black font-semibold' : 'border-[#e5e7eb] text-[#374151]'}`}>
                      ...
                      Subject
                      <span className={`ml-2 w-0 h-0 border-l-[4px] border-r-[4px] border-t-[4px] transition-transform ${openDropdown === 'courseSubject' ? 'rotate-180' : ''} ${selectedCourseSubjects.length > 0 ? 'border-t-black' : 'border-t-[#374151]'} border-l-transparent border-r-transparent`}></span>
                    </button>
                    <button onClick={() => toggleDropdown('coursePricing')} className={`px-4 py-1.5 border rounded-[30px] text-[12px] md:text-[13px] flex items-center transition-all dropdown-container bg-white ${coursePriceRange ? 'border-black text-black font-semibold' : 'border-[#e5e7eb] text-[#374151]'}`}>
                      ...
                      Pricing
                      <span className={`ml-2 w-0 h-0 border-l-[4px] border-r-[4px] border-t-[4px] transition-transform ${openDropdown === 'coursePricing' ? 'rotate-180' : ''} ${coursePriceRange ? 'border-t-black' : 'border-t-[#374151]'} border-l-transparent border-r-transparent`}></span>
                    </button>
                    ...
                    {(selectedCourseLevels.length > 0 || selectedCourseSubjects.length > 0 || coursePriceRange || courseNewlyLaunched || courseFastrackOnly || courseBestSellerOnly) && (
                      <button onClick={() => { setSelectedCourseLevels([]); setSelectedCourseSubjects([]); setCoursePriceRange(null); setCourseNewlyLaunched(false); setCourseFastrackOnly(false); setCourseBestSellerOnly(false); }} className="text-[#6366f1] text-[12px] md:text-[13px] font-medium whitespace-nowrap hover:underline">Reset Filters</button>
                    )}
                  </>
                ) : hasSubFilters ? (
                  <>
                    ...
                    {(activeTab === 'notes' || activeTab === 'pyqs' || activeTab === 'tools') && (
                      <button onClick={() => toggleDropdown('level')} className={`px-4 py-1.5 border rounded-[30px] text-[12px] md:text-[13px] flex items-center transition-all dropdown-container bg-white ${selectedLevel !== "Foundation" ? 'border-black text-black font-semibold' : 'border-[#e5e7eb] text-[#374151]'}`}>
                        Level
                        <span className={`ml-2 w-0 h-0 border-l-[4px] border-r-[4px] border-t-[4px] transition-transform ${openDropdown === 'level' ? 'rotate-180' : ''} ${selectedLevel !== "Foundation" ? 'border-t-black' : 'border-t-[#374151]'} border-l-transparent border-r-transparent`}></span>
                      </button>
                    )}
                    
                    {activeTab === 'notes' && (
                      <button onClick={() => toggleDropdown('notesSubject')} className={`px-4 py-1.5 border rounded-[30px] text-[12px] md:text-[13px] flex items-center transition-all dropdown-container bg-white ${selectedNotesSubjects.length > 0 ? 'border-black text-black font-semibold' : 'border-[#e5e7eb] text-[#374151]'}`}>
                        ...
                        Subject
                        <span className={`ml-2 w-0 h-0 border-l-[4px] border-r-[4px] border-t-[4px] transition-transform ${openDropdown === 'notesSubject' ? 'rotate-180' : ''} ${selectedNotesSubjects.length > 0 ? 'border-t-black' : 'border-t-[#374151]'} border-l-transparent border-r-transparent`}></span>
                      </button>
                    )}

                    {activeTab === 'pyqs' && (
                      <>
                        <button onClick={() => toggleDropdown('year')} className={`px-4 py-1.5 border rounded-[30px] text-[12px] md:text-[13px] flex items-center transition-all dropdown-container bg-white ${pyqYear ? 'border-black text-black font-semibold' : 'border-[#e5e7eb] text-[#374151]'}`}>
                          {pyqYear && <span className="w-5 h-5 bg-[#6366f1] text-white rounded-full text-[10px] flex items-center justify-center mr-2">1</span>}
                          Year
                          <span className={`ml-2 w-0 h-0 border-l-[4px] border-r-[4px] border-t-[4px] transition-transform ${openDropdown === 'year' ? 'rotate-180' : ''} ${pyqYear ? 'border-t-black' : 'border-t-[#374151]'} border-l-transparent border-r-transparent`}></span>
                        </button>
                        <button onClick={() => toggleDropdown('examType')} className={`px-4 py-1.5 border rounded-[30px] text-[12px] md:text-[13px] flex items-center transition-all dropdown-container bg-white ${examType ? 'border-black text-black font-semibold' : 'border-[#e5e7eb] text-[#374151]'}`}>
                          {examType && <span className="w-5 h-5 bg-[#6366f1] text-white rounded-full text-[10px] flex items-center justify-center mr-2">1</span>}
                          Exam Type
                          <span className={`ml-2 w-0 h-0 border-l-[4px] border-r-[4px] border-t-[4px] transition-transform ${openDropdown === 'examType' ? 'rotate-180' : ''} ${examType ? 'border-t-black' : 'border-t-[#374151]'} border-l-transparent border-r-transparent`}></span>
                        </button>
                      </>
                    )}

                    {activeTab === 'tools' && tools.map(tool => (
                      <button key={tool.id} onClick={() => setSelectedTool(tool.id)} className={`px-4 py-1.5 border rounded-[30px] text-[12px] md:text-[13px] whitespace-nowrap transition-all flex items-center gap-2 bg-white ${selectedTool === tool.id ? 'border-black text-black font-semibold' : 'border-[#e5e7eb] text-[#374151]'}`}>
                        {tool.label}
                    ...
                    {((activeTab === 'notes' && selectedNotesSubjects.length > 0) || (activeTab === 'pyqs' && (pyqYear || examType))) && (
                      <button onClick={() => { if (activeTab === 'notes') setSelectedNotesSubjects([]); if (activeTab === 'pyqs') { setPyqYear(null); setExamType(null); } }} className="text-[#6366f1] text-[12px] md:text-[13px] font-medium whitespace-nowrap hover:underline">Reset Filters</button>
                    )}
                  </>
                ) : (activeTab === 'news' || activeTab === 'dates') && (
                  <>
                    <button onClick={() => setSortOrder('recent')} className={`px-4 py-1.5 border rounded-[30px] text-[12px] md:text-[13px] whitespace-nowrap transition-all flex items-center gap-2 bg-white ${sortOrder === 'recent' ? 'border-black text-black font-semibold' : 'border-[#e5e7eb] text-[#374151]'}`}>
                      Recent First
                      {sortOrder === 'recent' && <X className="w-3.5 h-3.5 stroke-[2.5]" />}
                    </button>
                    <button onClick={() => setSortOrder('oldest')} className={`px-4 py-1.5 border rounded-[30px] text-[12px] md:text-[13px] whitespace-nowrap transition-all flex items-center gap-2 bg-white ${sortOrder === 'oldest' ? 'border-black text-black font-semibold' : 'border-[#e5e7eb] text-[#374151]'}`}>
                      Oldest First
            ...
            <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 relative">
              {openDropdown === 'branch' && (
                <div className="absolute top-0 left-4 right-4 sm:right-auto sm:left-6 lg:left-8 bg-white border border-[#e5e7eb] rounded-xl shadow-xl z-[9999] sm:min-w-[200px] max-w-[calc(100vw-2rem)] p-3 dropdown-container overflow-hidden">
                  {renderDropdownContent('branch')}
                </div>
              )}
              {openDropdown === 'level' && (
                <div className="absolute top-0 left-4 right-4 sm:right-auto sm:left-[120px] lg:left-[130px] bg-white border border-[#e5e7eb] rounded-xl shadow-xl z-[9999] sm:min-w-[160px] max-w-[calc(100vw-2rem)] p-3 dropdown-container overflow-hidden">
                  {renderDropdownContent('level')}
                </div>
              )}
              {openDropdown === 'notesSubject' && (
                <div className="absolute top-0 left-4 right-4 sm:right-auto sm:left-[230px] lg:left-[250px] bg-white border border-[#e5e7eb] rounded-xl shadow-xl z-[9999] sm:min-w-[200px] max-w-[calc(100vw-2rem)] p-3 dropdown-container overflow-hidden">
                  {renderDropdownContent('notesSubject')}
                </div>
              )}
              {openDropdown === 'year' && (
                <div className="absolute top-0 left-4 right-4 sm:right-auto sm:left-[230px] lg:left-[250px] bg-white border border-[#e5e7eb] rounded-xl shadow-xl z-[9999] sm:min-w-[140px] max-w-[calc(100vw-2rem)] p-3 dropdown-container overflow-hidden">
                  {renderDropdownContent('year')}
                </div>
              )}
              {openDropdown === 'examType' && (
                <div className="absolute top-0 left-4 right-4 sm:right-auto sm:left-[320px] lg:left-[350px] bg-white border border-[#e5e7eb] rounded-xl shadow-xl z-[9999] sm:min-w-[160px] max-w-[calc(100vw-2rem)] p-3 dropdown-container overflow-hidden">
              ...
              {openDropdown === 'courseSubject' && (
                <div className="absolute top-0 left-4 right-4 sm:right-auto sm:left-[210px] lg:left-[230px] bg-white border border-[#e5e7eb] rounded-xl shadow-xl z-[9999] sm:min-w-[180px] max-w-[calc(100vw-2rem)] p-3 dropdown-container overflow-hidden">
                  {renderDropdownContent('courseSubject')}
                </div>
              )}
              {openDropdown === 'coursePricing' && (
                <div className="absolute top-0 left-4 right-4 sm:right-auto sm:left-[310px] lg:left-[340px] bg-white border border-[#e5e7eb] rounded-xl shadow-xl z-[9999] sm:min-w-[140px] max-w-[calc(100vw-2rem)] p-3 dropdown-container overflow-hidden">
        ...
        <section className="py-8 bg-white min-h-[600px] relative z-0">
          <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
            ...
            {activeTab === "courses" && <PaidCoursesTab branch={selectedBranch} levels={selectedCourseLevels} subjects={selectedCourseSubjects} priceRange={coursePriceRange} newlyLaunched={courseNewlyLaunched} fasttrackOnly={courseFastrackOnly} bestSellerOnly={courseBestSellerOnly} />}
